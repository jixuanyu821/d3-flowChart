<template>
  <div class="dagre-graph-container" :id="containerId">
    <div class="zoom-div">
      <button v-for="item in directions" :key="item.prop" @click="direction = item.prop">{{item.label}}</button>
      <button class=".zoom" @click="zoomCtrl('in')">缩小</button>
      <button @click="zoomCtrl('out')">扩大</button>
    </div>
    <svg class="dagre">
      <g class="container"></g>  
    </svg> 
